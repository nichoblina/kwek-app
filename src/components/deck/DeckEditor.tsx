...
import type { Deck, SimpleCard, Category } from "@/lib/types";
import { generateId, getCategoryLabel } from "@/lib/utils";

const CATEGORIES: Category[] = ["general", "java", "spring", "jpa", "devsecops", "cs"];
...
export function DeckEditor({ deck, onSave, onCancel }: DeckEditorProps) {
  ...
  function saveCardForm() {
    ...
    if (cardForm.showQuiz) {
      cardForm.options.forEach((o, i) => {
        if (!o.trim()) cardErrors[`opt${i}`] = `Option ${LETTERS[i]} is required.`;
      });
    }
    ...
    const newCard: SimpleCard = {
      ...
      ...(cardForm.showQuiz && {
        options: cardForm.options.map((o) => o.trim()) as [string, string, string, string],
  ...
  const labelClass = "block text-[0.75rem] font-semibold text-muted uppercase tracking-wide mb-1.5 font-mono";
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-card border-[1.5px] border-border rounded-2xl p-6 mb-5">
        ...
        <div className="mb-4">
          ...
          />
          {errors.name && <p className="text-[0.75rem] text-primary mt-1">{errors.name}</p>}
        </div>
      ...
      {cards.length > 0 && (
        <div className="mb-5">
            ...
            {cards.map((card, idx) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-text-primary truncate">{card.front}</p>
                  <p className="text-xs text-muted truncate mt-0.5">{card.back}</p>
                  {card.options && (
                    <span
          ...
          {errors.cards && <p className="text-[0.75rem] text-primary mt-2">{errors.cards}</p>}
        </div>
      )}
      ...
      <div className="bg-card border-[1.5px] border-border rounded-2xl p-6 mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <textarea
              ...
              className={`${inputClass} resize-none`}
            />
            {errors.front && <p className="text-[0.75rem] text-primary mt-1">{errors.front}</p>}
          </div>
          <div>
            ...
            <textarea
              ...
              className={`${inputClass} resize-none`}
            />
            {errors.back && <p className="text-[0.75rem] text-primary mt-1">{errors.back}</p>}
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <select
            ...
            onChange={(e) => setCardForm((f) => ({ ...f, category: e.target.value as Category }))}
            className={inputClass}
          >
            {CATEGORIES.map((cat) => (
        ...
        <button
          ...
          style={{ color: cardForm.showQuiz ? "var(--color-secondary)" : "var(--color-muted)" }}
        >
          <span className="w-4 h-4 border-[1.5px] rounded flex items-center justify-center text-[0.65rem] transition-all"
            style={{
              borderColor: cardForm.showQuiz ? "var(--color-secondary)" : "var(--color-border)",
              background: cardForm.showQuiz ? "var(--color-secondary)" : "transparent",
          ...
          <div className="border-[1.5px] border-border rounded-xl p-4 mb-4">
                ...
                <div key={i}>
                  <label className={labelClass}>
                    ...
                    <input
                      ...
                      onChange={() => setCardForm((f) => ({ ...f, answerIndex: i as 0 | 1 | 2 | 3 }))}
                      className="ml-1"
                    />
                    {" "}(correct)
                  </label>
                  ...
                  />
                  {errors[`opt${i}`] && (
                    <p className="text-[0.75rem] text-primary mt-1">{errors[`opt${i}`]}</p>
                  )}
                </div>
              ))}
            </div>
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setCardForm((f) => ({ ...f, explanation: e.target.value }))}
